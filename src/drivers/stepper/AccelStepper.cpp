...
void AccelStepper::setOutputPins(uint8_t mask)
{
    ...
    uint8_t i;
    for (i = 0; i < numpins; i++)
  gpioPinWrite(_pin[i], (mask & (1 << i)) ? (_DEF_HIGH ^ _pinInverted[i]) : (_DEF_LOW ^ _pinInverted[i]));
}
...
{   
    ...
    if (_enablePin != 0xff)
    {
      gpioPinMode(_enablePin, _DEF_OUTPUT);
      gpioPinWrite(_enablePin, _DEF_LOW ^ _enableInverted);
    }
}
...
void    AccelStepper::enableOutputs()
{
    ...
    gpioPinMode(_pin[0], _DEF_OUTPUT);
    gpioPinMode(_pin[1], _DEF_OUTPUT);
    if (_interface == FULL4WIRE || _interface == HALF4WIRE)
    {
      gpioPinMode(_pin[2], _DEF_OUTPUT);
      gpioPinMode(_pin[3], _DEF_OUTPUT);
    }
    else if (_interface == FULL3WIRE || _interface == HALF3WIRE)
    {
      gpioPinMode(_pin[2], _DEF_OUTPUT);
    ...
    {
      gpioPinMode(_enablePin, _DEF_OUTPUT);
      gpioPinWrite(_enablePin, _DEF_HIGH ^ _enableInverted);
    }
}
